<?php
	...
	if (isset($_POST['pid'])) {
	  ...
	  $stmt = $conn->prepare('SELECT p_code FROM tbl_cart WHERE p_code=?');
	  ...
	  if (!$code) {
	    $query = $conn->prepare('INSERT INTO tbl_cart (p_name,p_price,p_img,qty,total_price,p_code) VALUES (?,?,?,?,?,?)');
	...
	if (isset($_GET['clear'])) {
	  $stmt = $conn->prepare('DELETE FROM tbl_cart');
	  $stmt->execute();
	  $_SESSION['showAlert'] = 'block';
	...
	if (isset($_POST['action']) && isset($_POST['action']) == 'order') {
	  ...
	  $stmt = $conn->prepare('INSERT INTO tbl_orders (name,email,phone,address,pmode,products,amount_paid)VALUES(?,?,?,?,?,?,?)');
	  $stmt->bind_param('sssssss',$name,$email,$phone,$address,$pmode,$products,$grand_total);
	  $stmt->execute();
	  $stmt2 = $conn->prepare('DELETE FROM tbl_cart');
	  $stmt2->execute();
